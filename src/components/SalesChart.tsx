...
const SalesChart: React.FC = () => {
  ...
  useEffect(() => {
   ...
   sales.forEach(sale => {
  // Type guard para garantir que properties existe e tem price
  const properties = sale.properties as { price: number }[] | null;
  const price = properties && properties.length > 0 ? properties[0].price : null;
  if (price) {
